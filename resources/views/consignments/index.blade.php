    ...
    <table class="table table-sm table-hover">
      ...
      @forelse($rows as $r)
        <tr>
          ...
          <td class="num">{{ number_format((float)$r->remaining_kg,1) }}</td>
          <td><span class="badge bg-{{ $r->is_ready ? 'success' : ($r->status==='rejected'?'danger':'secondary') }}">{{ $r->status_name }}</span></td>
